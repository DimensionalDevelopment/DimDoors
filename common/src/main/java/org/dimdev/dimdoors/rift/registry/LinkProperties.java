...
public class LinkProperties {
    ...
    private static final Codec<Set<Integer>> GROUPS_CODEC = Codec.INT_STREAM.xmap(
            stream -> stream.boxed().collect(Collectors.toSet()),
            groups -> groups.stream().mapToInt(Integer::intValue)
    );
    public static final Codec<LinkProperties> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.FLOAT.optionalFieldOf("floatingWeight", 0.0f).forGetter(properties -> properties.floatingWeight),
            Codec.FLOAT.optionalFieldOf("entranceWeight", 0.0f).forGetter(LinkProperties::getEntranceWeight),
            GROUPS_CODEC.optionalFieldOf("groups", Collections.emptySet()).forGetter(LinkProperties::getGroups),
