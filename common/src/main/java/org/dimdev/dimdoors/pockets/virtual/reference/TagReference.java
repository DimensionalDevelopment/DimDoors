...
public class TagReference extends PocketGeneratorReference {
    ...
    public TagReference(Optional<Expression> weight, Optional<Boolean> setupLoot, List<String> required, List<String> blackList, boolean exact) {
        super(weight);
	...
	private Holder<PocketGenerator> selectPocket(PocketGenerationContext parameters, boolean peek) {
		if (pockets == null) pockets = getPocketsMatchingTags(parameters.provider().lookupOrThrow(ModRegistries.POCKET_GENERATOR).listElements(), required, blackList, exact != null && exact);
