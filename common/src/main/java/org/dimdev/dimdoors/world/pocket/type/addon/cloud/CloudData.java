...
public interface CloudData {
    ...
    public record CloudDataType<T extends CloudData>(MapCodec<T> codec, StreamCodec<RegistryFriendlyByteBuf, T> streamCodec) {
        private static final Map<Identifier, CloudDataType<?>> ID_TO_TYPE = new HashMap<>();
        private static final Map<CloudDataType<?>, Identifier> TYPE_TO_ID = new HashMap<>();

        public static Codec<CloudDataType<?>> CODEC = Identifier.CODEC.xmap(ID_TO_TYPE::get, TYPE_TO_ID::get);
        public static StreamCodec<RegistryFriendlyByteBuf, CloudDataType<?>> STREAM_CODEC = Identifier.STREAM_CODEC.<RegistryFriendlyByteBuf>cast().map(ID_TO_TYPE::get, TYPE_TO_ID::get);
        ...
        public static <U extends CloudData> CloudDataType<U> register(Identifier id, MapCodec<U> codec, StreamCodec<RegistryFriendlyByteBuf, U> streamCodec) {
