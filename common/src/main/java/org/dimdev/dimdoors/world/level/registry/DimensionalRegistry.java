...
public class DimensionalRegistry extends SavedData {
	...
	private static Factory<? extends DimensionalRegistry> FACTORY = new Factory<>(DimensionalRegistry::new, DimensionalRegistry::of, null);
	private final Map<ResourceKey<Level>, PocketDirectory> pocketRegistry;
	private final RiftRegistry riftRegistry;

    private final PrivateRegistry privateRegistry;
	...
	public static DimensionalRegistry of(CompoundTag nbt, HolderLookup.Provider provider) {
		...
		var pocketRegistry = futurePocketRegistry.join();
		...
		var riftRegistry = futureRiftRegistry.join();

		var privateRegistry = futurePrivateRegistry.join();
	...
	public static DimensionalRegistry getDimensionalRegistry() {
		return DimensionalDoors.getServer().overworld().getDataStorage().computeIfAbsent(FACTORY, "dimdoors:dimensional_registry");
	}
