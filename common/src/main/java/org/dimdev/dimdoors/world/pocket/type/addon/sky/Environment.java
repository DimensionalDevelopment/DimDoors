...
public interface Environment {
    ...
    public record EnvironmentType<T extends Environment>(MapCodec<T> codec, StreamCodec<RegistryFriendlyByteBuf, T> streamCodec) {
        private static final Map<Identifier, EnvironmentType<?>> ID_TO_TYPE = new HashMap<>();
        private static final Map<EnvironmentType<?>, Identifier> TYPE_TO_ID = new HashMap<>();

        public static Codec<EnvironmentType<?>> CODEC = Identifier.CODEC.xmap(ID_TO_TYPE::get, TYPE_TO_ID::get);
        public static StreamCodec<RegistryFriendlyByteBuf, EnvironmentType<?>> STREAM_CODEC = Identifier.STREAM_CODEC.<RegistryFriendlyByteBuf>cast().map(ID_TO_TYPE::get, TYPE_TO_ID::get);
        ...
        public static <U extends Environment> EnvironmentType<U> register(Identifier id, MapCodec<U> codec, StreamCodec<RegistryFriendlyByteBuf, U> streamCodec) {
