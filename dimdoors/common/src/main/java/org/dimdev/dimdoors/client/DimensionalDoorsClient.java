...
public class DimensionalDoorsClient implements ModClient<IDimDoorsClientSided<?>> {
    ...
    public void init(IDimDoorsClientSided<?> sided) {
        ...
        DimCore.clientPlatform().onClientPlayerJoin(ClientPacketListener::clearPocketAddons);
        registerCompats();
        EnvironmentAddonClient.init();

        sided.onPreRender(this::preRender);

        if(DimCore.platform().isModLoaded("imguimc")) {
    ...
    private static void registerCompats() {
        if (DimCore.platform().isModLoaded("iris") || DimCore.platform().isModLoaded("oculus")) detector = new IrisCompat();
    }
    ...
    public static void initGeneratedDoorCutouts() {
        ...
        if (generatedBlocks.length > 0) {
            DimCore.clientPlatform().register(RenderType.cutout(), generatedBlocks);
        }
    }

    public void delayedInit() {
        initGeneratedDoorCutouts();
        DimCore.clientPlatform().register(RenderType.cutout(), ModBlocks.QUARTZ_DOOR, ModBlocks.GOLD_DOOR, ModBlocks.DRIFTWOOD_LEAVES, ModBlocks.DRIFTWOOD_SAPLING, ModBlocks.DRIFTWOOD_DOOR, ModBlocks.DRIFTWOOD_TRAPDOOR, ModBlocks.UNRAVELED_SPIKE, ModBlocks.DRIFTWOOD_DOOR, ModBlocks.DIALING_DOOR);
    }
