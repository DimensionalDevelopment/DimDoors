...
public class TesselatingRecipeProvider {
	public static void generate(Consumer<FinishedRecipe> exporter) {
		...
		TesselatingRecipeJsonBuilder.create(ModBlocks.BLACK_FABRIC.get()).pattern("XX").pattern("XO").input('O', ModItems.STABLE_FABRIC.get()).input('X', ModItems.WORLD_THREAD.get()).criterion("stable_fabric", TriggerInstance.hasItems(ModItems.STABLE_FABRIC.get())).offerTo(exporter, DimensionalDoors.id("fabric_of_reality"));
