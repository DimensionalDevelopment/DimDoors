...
public class EventHookContainer {
	...
	public EventHookContainer(DDProperties properties) {this.properties = properties;}
	...
	public void onPlayerEvent(PlayerInteractEvent event) {
		...
		if (stack != null) {
			...
			if (BaseItemDoor.tryToPlaceDoor(stack, event.entityPlayer, world, event.pos, event.face))
	...
	public void onWorldLoad(WorldEvent.Load event) {
		...
		if (!event.world.isRemote && !PocketManager.isLoaded()) PocketManager.load();
	}

	@SubscribeEvent
	public void onPlayerFall(LivingFallEvent event) {
		event.setCanceled(event.entity.worldObj.provider.getDimensionId() == properties.LimboDimensionID);
	}
	...
	private boolean isValidSourceForLimbo(WorldProvider provider) {
		...
		return (worldProperties.UniversalLimboEnabled && provider.getDimensionId() != properties.LimboDimensionID) ||
	...
	public void onChunkLoad(ChunkEvent.Load event) {
		...
		if (!chunk.getWorld().isRemote && PocketManager.isLoaded()) {
			DimData dimension = PocketManager.createDimensionData(chunk.getWorld());
