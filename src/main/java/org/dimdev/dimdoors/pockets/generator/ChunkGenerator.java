...
public class ChunkGenerator extends PocketGenerator {
	...
	public Pocket prepareAndPlacePocket(PocketGenerationContext parameters, Pocket.PocketBuilder<?, ?> builder) {
		...
		for (ChunkAccess protoChunk : protoChunks) {
			try {
				genWorldChunkGenerator.fillFromNoise(net.minecraft.Util.backgroundExecutor(), Blender.empty(), config, genWorld.structureManager().forWorldGenRegion(protoRegion), protoChunk).get();
		...
		*/
		world.setBlockAndUpdate(world.getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, pocket.getOrigin()), ModBlocks.DETACHED_RIFT.defaultBlockState());

		DetachedRiftBlockEntity rift = ModBlockEntityTypes.DETACHED_RIFT.create(world.getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, pocket.getOrigin()), ModBlocks.DETACHED_RIFT.defaultBlockState());
