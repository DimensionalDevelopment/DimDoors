...
public abstract class GUIConfigScreen extends GuiScreen {
     ...
     */
    protected static final ResourceLocation BG_TEXTURE = new ResourceLocation("textures/gui/container/inventory.png");
    /**
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); ++i1) {
            ...
            if (slot.isEnabled()) this.drawSlot(slot);
        ...
        if (!itemstack.isEmpty()) {
            ...
            if (!this.draggedStack.isEmpty() && this.isRightMouseClick) {
                ...
                if (itemstack.isEmpty()) s = "" + TextFormatting.YELLOW + "0";
            }
    ...
    private void drawSlot(Slot slotIn) {
        ...
        if (itemstack.isEmpty() && slotIn.isEnabled()) {
            ...
            if (Objects.nonNull(textureatlassprite)) {
                GlStateManager.disableLighting();
    ...
    private Slot getSlotAtPosition(int x, int y) {
        for (int i = 0; i < this.inventorySlots.inventorySlots.size(); ++i) {
            ...
            if (this.isMouseOverSlot(slot, x, y) && slot.isEnabled()) return slot;
        }
        return null;
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0 || mouseButton == 1 || flag) {
            ...
            if (Objects.nonNull(slot)) flag1 = false; // Forge, prevent dropping of items through slots outside of GUI boundaries
            int l = -1;
            if (Objects.nonNull(slot)) l = slot.slotNumber;
            if (flag1) l = -999;
            ...
            if (l != -1) {
                if (this.mc.gameSettings.touchscreen) {
                    if (Objects.nonNull(slot) && slot.getHasStack()) {
                        this.clickedSlot = slot;
                        this.draggedStack = ItemStack.EMPTY;
                        this.isRightMouseClick = mouseButton == 1;
                    } else this.clickedSlot = null;
                    ...
                    if (this.mc.player.inventory.getItemStack().isEmpty()) {
                        if (this.mc.gameSettings.keyBindPickBlock.isActiveAndMatches(mouseButton - 100))
                            ...
                            } else if (l == -999) clicktype = ClickType.THROW;
                        ...
                        if (mouseButton == 0) this.dragSplittingLimit = 0;
                        else if (mouseButton == 1) this.dragSplittingLimit = 1;
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        int k = -1;
        if (Objects.nonNull(slot)) k = slot.slotNumber;
        if (flag) k = -999;
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(ItemStack.EMPTY, slot)) {
            ...
            } else this.handleMouseClick(slot, k, state, ClickType.PICKUP_ALL);
            ...
            if (Objects.nonNull(this.clickedSlot) && this.mc.gameSettings.touchscreen) {
                if (state == 0 || state == 1) {
                    ...
                    if (k != -1 && !this.draggedStack.isEmpty() && flag2) {
                        ...
                        if (this.mc.player.inventory.getItemStack().isEmpty()) this.returningStack = ItemStack.EMPTY;
                        else {
                            this.handleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, state, ClickType.PICKUP);
                ...
                this.handleMouseClick(null, -999,
                        Container.getQuickcraftMask(0, this.dragSplittingLimit), ClickType.QUICK_CRAFT);
                ...
                this.handleMouseClick(null, -999,
                        Container.getQuickcraftMask(2, this.dragSplittingLimit), ClickType.QUICK_CRAFT);
        ...
        if (this.mc.player.inventory.getItemStack().isEmpty()) this.lastClickTime = 0L;
        this.dragSplitting = false;
    }
    ...
    protected void handleMouseClick(Slot slotIn, int slotId, int mouseButton, ClickType type) {
        if (Objects.nonNull(slotId)) slotId = slotIn.slotNumber;
     ...
     */
    protected void checkHotbarKeys(int keyCode) {
        if (this.mc.player.inventory.getItemStack().isEmpty() && Objects.nonNull(this.hoveredSlot)) {
            for (int i = 0; i < 9; ++i) {
    ...
    public void onGuiClosed() {
        if (Objects.nonNull(this.mc.player)) this.inventorySlots.onContainerClosed(this.mc.player);
    }
